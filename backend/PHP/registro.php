...
if (isset($_POST['enviar']) && $_POST['enviar'] == 'Registrarse') {
    if (!empty($_POST['usuarioNombre']) && !empty($_POST['usuarioApellido']) && !empty($_POST['nacimiento']) && !empty($_POST['Correo']) && !empty($_POST['Telefono']) && !empty($_POST['contrasena'])) {
        ...
        $sql = "INSERT INTO `users`(`email`, `password_hash`, `first_name`, `last_name`, `phone`, `date_of_birth`, `created_at`, `updated_at`, `is_active`) VALUES ('$correo','$contrasena','$nombre','$apellido','$telefono','$nacimiento','$fecha_creacion','$fecha_actualizado','$activo')";
