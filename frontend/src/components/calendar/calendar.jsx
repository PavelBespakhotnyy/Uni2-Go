...
export default function Calendar() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#f5f5f5] text-[#1a1a1a] font-sans overflow-hidden">
      <header className="w-full flex items-center justify-evenly py-8 bg-white border-b-2 border-[#1a1a1a] z-50 shrink-0 px-0">
        <h1 className="text-2xl font-black uppercase tracking-tight whitespace-nowrap m-0">
          {format(currentDate, 'MMMM yyyy', { locale: es })}
        </h1>

        <div className="flex items-center gap-2 bg-[#f0f0f0] p-1.5 rounded-lg border border-gray-200 shrink-0">
        ...
        <div className="flex bg-[#f0f0f0] p-1.5 rounded-lg border border-gray-200 shrink-0">
          {Object.values(VIEWS).map((v) => (
            <button key={v} onClick={() => setView(v)} className={`px-6 py-2 text-xs font-black rounded-md transition-all uppercase tracking-widest ${view === v ? 'bg-white text-[#1a1a1a] shadow-sm border border-gray-200' : 'text-gray-500 hover:text-[#1a1a1a]'}`}>{v === VIEWS.MONTH ? 'Mes' : v === VIEWS.WEEK ? 'Semana' : 'Día'}</button>
          ))}
        </div>

        <button onClick={() => openAddModal(currentDate)} className="flex items-center bg-[#1a1a1a] text-white px-10 py-3.5 rounded-lg font-black hover:bg-black transition-all shadow-md uppercase text-xs tracking-[0.2em] shrink-0"><span>Nuevo</span></button>
      </header>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-md shadow-2xl w-full max-w-lg overflow-hidden border-2 border-[#1a1a1a] animate-in fade-in zoom-in duration-150">
            <div className="flex items-center justify-between p-5 bg-[#dbe4eb] border-b-2 border-[#1a1a1a]">
              <h2 className="text-sm font-black flex items-center gap-3 text-[#1a1a1a] uppercase tracking-widest"><div className="bg-[#1a1a1a] text-white p-1.5 rounded">{selectedEvent ? <Edit2 size={14}/> : <Plus size={14}/>}</div>{selectedEvent ? 'Editar Evento' : 'Nuevo Evento'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-1 hover:bg-black/10 rounded transition-colors text-[#1a1a1a]"><X size={24} /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6 bg-white">
              ...
              <div className="space-y-4">
                <div className="relative"><label className="block text-[10px] font-black mb-1.5 text-gray-400 uppercase tracking-widest">Título</label><input type="text" required className="w-full border-b-2 border-gray-100 py-2 focus:border-[#1a1a1a] outline-none transition-all text-lg font-bold" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="¿Qué vas a hacer?" /></div>
                <div className="relative"><label className="block text-[10px] font-black mb-1.5 text-gray-400 uppercase tracking-widest flex items-center gap-1.5"><AlignLeft size={12}/> Descripción</label><textarea rows="2" className="w-full border border-gray-200 rounded p-3 focus:border-[#1a1a1a] outline-none transition-all text-sm resize-none bg-gray-50" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} /></div>
              </div>
              
              <div className="bg-gray-50 p-5 rounded-md border border-gray-100 space-y-4">
                <div className="flex items-center justify-between"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2"><Clock size={12}/> Tiempo</span><label className="flex items-center gap-2 cursor-pointer group"><input type="checkbox" checked={formData.allDay} onChange={(e) => setFormData({ ...formData, allDay: e.target.checked })} className="w-4 h-4 cursor-pointer accent-[#1a1a1a]" /><span className="text-xs font-bold text-gray-600 group-hover:text-[#1a1a1a]">Todo el día</span></label></div>
                {formData.allDay ? (
                  <div className="flex gap-4 animate-in slide-in-from-top-1"><div className="flex-1"><label className="block text-[9px] font-bold text-gray-400 mb-1 uppercase">Fecha</label><div className="relative"><CalendarDays className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} /><input type="date" required className="w-full bg-white border border-gray-300 rounded px-10 py-2.5 focus:border-[#1a1a1a] outline-none font-bold text-sm" value={formData.start.split('T')[0]} onChange={(e) => handleDateChange('start', e.target.value)} /></div></div></div>
                ) : (
                  <div className="space-y-4 animate-in slide-in-from-top-1">
                    <div className="grid grid-cols-[1fr_120px] gap-4"><div><label className="block text-[9px] font-bold text-gray-400 mb-1 uppercase">Inicio</label><input type="date" required className="w-full bg-white border border-gray-300 rounded px-3 py-2.5 focus:border-[#1a1a1a] outline-none font-bold text-sm" value={formData.start.split('T')[0]} onChange={(e) => handleDateChange('start', e.target.value)} /></div><div><label className="block text-[9px] font-bold text-gray-400 mb-1 uppercase">Hora</label><input type="time" required className="w-full bg-white border border-gray-300 rounded px-3 py-2.5 focus:border-[#1a1a1a] outline-none font-bold text-sm" value={formData.start.split('T')[1] || '09:00'} onChange={(e) => handleTimeChange('start', e.target.value)} /></div></div>
                    <div className="grid grid-cols-[1fr_120px] gap-4"><div><label className="block text-[9px] font-bold text-gray-400 mb-1 uppercase">Fin</label><input type="date" required className="w-full bg-white border border-gray-300 rounded px-3 py-2.5 focus:border-[#1a1a1a] outline-none font-bold text-sm" value={formData.end.split('T')[0]} onChange={(e) => handleDateChange('end', e.target.value)} /></div><div><label className="block text-[9px] font-bold text-gray-400 mb-1 uppercase">Hora</label><input type="time" required className="w-full bg-white border border-gray-300 rounded px-3 py-2.5 focus:border-[#1a1a1a] outline-none font-bold text-sm" value={formData.end.split('T')[1] || '10:00'} onChange={(e) => handleTimeChange('end', e.target.value)} /></div></div>
                  </div>
                )}
              </div>
...
function WeekView({ currentDate, events, onEventClick }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 calendar-grid-scrollable relative bg-white min-h-0">
