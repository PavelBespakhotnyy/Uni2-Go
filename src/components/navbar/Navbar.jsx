...
export default function Navbar() {
  ...
  // Sync body classes (CSS relies on body.sidebar-collapsed / body.sidebar-ready)
  useEffect(() => {
    if (collapsed) {
  ...
  const navItems = [
    ...
    { to: '/user',           icon: 'bx-user',         label: 'Usuario' },
  ];
    ...
    <nav className={`sidebar${collapsed ? ' collapsed' : ''}`} id="navbar">
      ...
      <ul className="buttons-list">
        {navItems.map(({ to, icon, label, badge }) => (
          <li key={to} className={label.toLowerCase().replace(' ', '-')}>
            <NavLink
              to={to}
